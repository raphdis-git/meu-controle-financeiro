...
$('#status').onchange=payment;$('#manual').oninput=bill;$('#close').onclick=()=>$('#editor').close();$('#year').onchange=render;$('#month').onchange=()=>{selectedMonth=Number($('#month').value);render()};$('#add').onclick=()=>{$('#new-error').textContent='';$('#new-fixed').checked=false;$('#new-fixed-start').value=`${$('#year').value}-${String(selectedMonth+1).padStart(2,'0')}`;$('#new-fixed-value').value='';$('#new-fixed-day').value='10';setupNewRepeat();$('#new').showModal()};$('#cancelnew').onclick=()=>$('#new').close();$('#newform').onsubmit=e=>{
 e.preventDefault();const name=$('#newname').value.trim();if(!name)return;
 const row={id:Math.max(0,...rows.map(r=>r.id))+1,name,group:Number($('#group').value)};
 const reopened=[];
 if($('#new-fixed').checked&&row.group!==7){
  let parsed;try{parsed=parseExpectedValue($('#new-fixed-value').value)}catch(error){$('#new-error').textContent=error.message;return}
  const start=$('#new-fixed-start').value,day=Number($('#new-fixed-day').value);
  if(parsed.value===null||!/^\d{4}-(0[1-9]|1[0-2])$/.test(start)||!Number.isInteger(day)||day<1||day>31){$('#new-error').textContent='Informe o valor, o mês de início e um dia entre 1 e 31.';return}
  const [y,m]=start.split('-').map(Number);row.recurrences=[{start:y*12+m-1,end:null,value:parsed.value,formula:parsed.formula,day}];
  if(!Array.from($('#year').options).some(o=>Number(o.value)===y))$('#year').add(new Option(String(y),String(y)));
 }
 rows.push(row);reopenRecurringMonths(row,reopened);$('#new').close();$('#newform').reset();setupNewRepeat();render();toast(reopened.length?'Conta criada. Meses reabertos: '+reopened.map(periodLabel).join(', '):'Conta cadastrada.');
};render();
...
function setupNewRepeat(){const card=Number($('#group').value)===7;$('#new-repeat').hidden=card;$('#new-fixed-fields').hidden=card||!$('#new-fixed').checked}
$('#new-fixed').onchange=setupNewRepeat;$('#group').onchange=setupNewRepeat;
...
function applyRepeatEdit(reopened){
 ...
 row.recurrences.push({start:period,end:null,value:draft.value,formula:draft.valueFormula,day});
