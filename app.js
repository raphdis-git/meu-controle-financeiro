...
function key(r,m,y=$('#year').value){return `${r.id}-${y}-${m}`}function blank(){return{value:null,description:'',manual:null,paid:false,actual:null,due:'',paiddate:'',purchases:[]}}function baseRec(r,m,y){const saved=records[key(r,m,y)];if(saved)return saved;const period=Number(y??$('#year').value)*12+m,rule=recurrenceAt(r,period);return rule?{...blank(),value:rule.value,manual:r.group===7?rule.value:null,valueFormula:rule.formula,description:rule.description??'',due:recurringDue(period,rule.day)}:blank()}function rec(r,m,y){
...
function deleteEntries(future){
  ...
  if(!future&&(recurrenceAt(row,cutoff)||(row.cardRecurrences||[]).some(r=>cutoff>=r.start&&(r.end===null||cutoff<=r.end))))records[key(row,m,year)]={...blank(),excluded:true};
...
function stopRecurrences(row,period){row.cardRecurrences=(row.cardRecurrences||[]).filter(r=>r.start<period).map(r=>({...r,end:r.end===null?period-1:Math.min(r.end,period-1)}));row.recurrences=(row.recurrences||[]).filter(r=>r.start<period).map(r=>({...r,end:r.end===null?period-1:Math.min(r.end,period-1)}))}
